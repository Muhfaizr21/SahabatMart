...
export default function HeroSlider() {
  ...
  return (
    <section className="relative overflow-hidden min-h-[480px] flex items-center" style={{ backgroundColor: slide.bg_color || slide.bg || '#3b82f6', transition: 'background-color 0.6s ease' }}>
      <div className="max-w-7xl mx-auto px-6 w-full">
        <div className="flex items-center gap-8 py-16">
          <div className="flex-1 z-10 text-left">
            <span className="inline-block bg-white/20 text-white text-sm px-4 py-1.5 rounded-full mb-5 font-bold">{slide.badge || 'PROMO'}</span>
            <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight mb-4 max-w-lg">{slide.title}</h1>
            <p className="text-white/80 text-lg mb-8">
              {slide.subtitle || 'Penawaran eksklusif'} <span className="text-yellow-300 font-bold text-2xl">{slide.offer}</span>
            </p>
          ...
          <div className="flex-1 hidden md:flex justify-end">
            <img src={formatImage(slide.image)} alt={slide.title} className="h-80 lg:h-96 w-auto object-cover rounded-2xl shadow-2xl" style={{ transition: 'all 0.5s ease' }} />
          </div>
        </div>
      </div>
