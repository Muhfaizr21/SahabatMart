...
export default function DiagnosticCTA({ data }) {
  return (
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
            <div className="flex-1 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 text-white rounded-full text-[10px] font-black uppercase tracking-widest mb-6 border border-white/10 backdrop-blur-md">
                {data?.label || 'Smart Diagnostic'}
              </div>
              <h2 className="text-4xl md:text-5xl font-black text-white leading-tight mb-6">
                {(data?.title || 'Tampil Percaya Diri dengan Kulit Impian Anda.').split(data?.highlight || 'Kulit Impian').map((part, i, arr) => (
                  <React.Fragment key={i}>
                    {part}
                    {i < arr.length - 1 && <span className="text-amber-400 italic">{data?.highlight || 'Kulit Impian'}</span>}
                  </React.Fragment>
                ))}
              </h2>
              <p className="text-white/70 text-lg mb-10 max-w-xl mx-auto lg:mx-0 font-medium">
                {data?.desc || 'Bingung memilih produk yang tepat? Ikuti tes kesehatan kulit kami dan dapatkan rekomendasi produk yang dipersonalisasi khusus untuk Anda.'}
              </p>
            ...
            <div className="flex-1 relative hidden lg:block">
              <div className="relative z-10 rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white/20">
                <img 
                  src={data?.image || "/skincare_diagnostic.webp"} 
                  alt={data?.label || "Skin Care Diagnostic"} 
