...
const BusinessOpportunity = ({ previewData }) => {
  ...
  const getSimulationData = () => {
    if (cmsContent && cmsContent.simulations && Array.isArray(cmsContent.simulations)) {
      const parsed = {};
      cmsContent.simulations.forEach(sim => {
        parsed[sim.id || 5] = {
          title: sim.title,
          desc: sim.desc,
          premium: sim.premium,
          free: sim.free,
          levels: [
            { name: sim.level_1_name, income: sim.level_1_income },
            { name: sim.level_2_name, income: sim.level_2_income },
            { name: sim.level_3_name, income: sim.level_3_income },
            { name: sim.level_4_name, income: sim.level_4_income },
            { name: sim.level_5_name, income: sim.level_5_income },
          ]
        };
      });
      return parsed;
  ...
  return (
    <div className="min-h-screen overflow-hidden" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="bg-[#C62828] text-white pt-28 pb-20 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <span className="inline-block bg-[#FFC107] text-gray-900 text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-wider mb-6">
             {str('hero_title', 'Peluang Bisnis Skincare Resmi')}
          </span>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mb-4 leading-tight">
            {str('hero_headline', 'Pelanggan Jadi Asetmu, Komisi Masuk Selamanya').split(',').map((part, i, arr) => (
          ...
          <p className="text-white/80 text-base md:text-lg max-w-2xl mx-auto mb-10 leading-relaxed">
            {str('hero_subtitle', 'Bisnis skincare premium tanpa modal, tanpa stok. Cukup bagikan link, ajak mitra, dan dapatkan penghasilan jutaan rupiah setiap bulan — bahkan saat tidur sekalipun.')}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
               {str('hero_cta_text', 'Daftar Gratis Sekarang')}
            </Link>
            <a
              href="#simulasi"
            ...
            >
              {str('hero_cta_secondary', 'Lihat Simulasi Komisi')}
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#FFC107] py-16 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">{str('difference_title', 'Kenapa Bisnis Ini Berbeda?')}</h2>
            <p className="text-gray-700 text-sm">{str('difference_subtitle', 'Tidak ada bisnis lain yang semudah dan sefleksibel ini.')}</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(cmsContent?.difference_items || [
      ...
      <section className="bg-[#FDF8F3] py-16 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">{str('how_it_works_title', 'Cara Kerjanya Sangat Simpel')}</h2>
            <p className="text-gray-500 text-sm">{str('how_it_works_subtitle', 'Hanya 4 langkah untuk mulai menghasilkan komisi')}</p>
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            {(cmsContent?.how_it_works_items || [
      ...
      <section className="bg-[#C62828] py-16 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-white mb-2">{str('pricing_title', 'Struktur Komisi Member')}</h2>
            <p className="text-white/70 text-sm">{str('pricing_subtitle', 'Pilih paket yang sesuai dengan target penghasilan Anda')}</p>
          </div>
      ...
      <section id="simulasi" className="bg-[#FDF8F3] py-16 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">{str('simulation_title', 'Simulasi Potensi Komisi')}</h2>
            <p className="text-gray-500 text-sm">{str('simulation_subtitle', 'Lihat berapa yang bisa Anda hasilkan dengan sistem duplikasi')}</p>
          </div>
      ...
      <section className="bg-white py-16 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">{str('target_title', 'Cocok untuk Siapa?')}</h2>
            <p className="text-gray-500 text-sm">{str('target_subtitle', 'Bisnis ini terbuka untuk semua kalangan')}</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {(cmsContent?.target_items || [
      ...
      <section className="bg-[#C62828] py-20 px-6 text-center relative overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight">
            {str('cta_final_title', 'Saatnya Bangun Masa Depanmu!').split(' ').map((word, i, arr) => (
              <React.Fragment key={i}>
          ...
          <p className="text-white/75 text-base mb-10 max-w-lg mx-auto leading-relaxed">
            {str('cta_final_subtitle', 'Tidak perlu modal besar untuk memulai perubahan. Cukup gunakan produknya, bagikan linknya, dan nikmati komisinya.')}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to="/register"
            ...
            >
               {str('cta_final_text', 'GABUNG SEKARANG')}
            </Link>
            <Link
              to="/shop"
              className="w-full sm:w-auto px-10 py-4 bg-white/10 border border-white/30 text-white font-bold text-base rounded-full hover:bg-white/20 transition-all duration-300"
            >
               {str('cta_final_secondary', 'Lihat Produk Dulu')}
            </Link>
          </div>
        </div>
      </section>
