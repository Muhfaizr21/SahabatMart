...
export default function AdminCommissions() {
  ...
  const EMPTY = { id: 0, category_name: '', merchant_id: '', fee_percent: 0.05, note: '' };
  ...
  const save = (e) => {
    ...
    // Preparation for API
    const payload = { ...modal };
    if (modalType === 'category') delete payload.merchant_id;
        ...
        <Modal title={`${modal.id ? 'Edit' : 'Buat'} Commission ${modalType === 'category' ? 'Category' : 'Merchant'}`} onClose={() => setModal(null)}>
          <form onSubmit={save}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <FieldLabel>{modalType === 'category' ? 'Target Kategori' : 'Target Merchant'}</FieldLabel>
                {modalType === 'category' ? (
                  <select 
                    style={{ ...A.select, width: '100%' }} 
                    value={modal.category_name} 
                    onChange={e => setModal(p => ({ ...p, category_name: e.target.value }))}
                  >
                    <option value="">— Pilih Kategori Prod —</option>
                    {categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                  </select>
                ) : (
                  <select 
                    style={{ ...A.select, width: '100%' }} 
                    value={modal.merchant_id} 
                    onChange={e => setModal(p => ({ ...p, merchant_id: e.target.value }))}
                  >
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input 
                    ...
                    style={{ ...A.select, width: '100%', paddingRight: 36 }} 
                    value={(modal.fee_percent * 100).toFixed(2)} 
                    onChange={e => setModal(p => ({ ...p, fee_percent: (parseFloat(e.target.value) || 0) / 100 }))} 
                  />
                  <div style={{ position: 'absolute', right: 12, top: 12, fontWeight: 700, color: '#94a3b8' }}>%</div>
                </div>
                <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>Contoh: Masukkan 5 untuk 5% biaya layanan.</p>
              </div>

              {modalType === 'merchant' && (
                <div>
                   <FieldLabel>Catatan Khusus (Optional)</FieldLabel>
                   <textarea 
                    style={{ ...A.select, width: '100%', minHeight: 80, paddingTop: 10 }}
                    placeholder="Alasan override komisi..."
                    value={modal.note}
