...
export default function AdminDemographics() {
  ...
  useEffect(() => {
    ...
    window.L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.webp', {
                ...
                <div className="space-y-6">
                  <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
                    <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <i className="bx bx-show-alt text-lg text-rose-500" />
                      Aktivitas Halaman per Kota
                    </h3>
                    {stats?.city_avg_pages?.length === 0 ? (
                      <p className="text-slate-400 text-xs text-center py-6">Belum ada data</p>
                    ) : (
                      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                      {(() => {
                        const validCities = (stats?.city_avg_pages || []).filter(c => c.city && c.city !== 'Unknown');
                        const maxHits = Math.max(...validCities.map(c => c.total_hits || 0), 1);
                        return validCities.map((city, i) => (
                          <div key={i} className="flex flex-col gap-1">
                            <div className="flex justify-between text-xs font-bold">
                              <span className="text-slate-700">{city.city}</span>
                              <span className="text-rose-600">{city.total_hits || 0} dilihat</span>
                            </div>
                            <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                              <div 
                                style={{ width: `${Math.max(2, Math.min((city.total_hits || 0) / maxHits * 100, 100))}%` }} 
